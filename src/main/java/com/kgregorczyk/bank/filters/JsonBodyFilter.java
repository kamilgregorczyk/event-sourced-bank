...
public class JsonBodyFilter implements Filter {
  ...
  private static final ImmutableSet<String> HTTP_METHODS_WITH_BODY =
      ImmutableSet.of("POST", "PUT", "PATCH");
  ...
  public void handle(Request request, Response response) {
    ...
    if (HTTP_METHODS_WITH_BODY.contains(request.requestMethod())) {
      if (request.body().isEmpty()) {
        halt(HTTP_BAD_REQUEST, new APIResponse(Status.ERROR, "Body is empty").toJson());
